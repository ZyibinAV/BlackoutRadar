...
class DuplicateResolverTest {
    ...
    @BeforeEach
    void setUp() {
        resolver = new DuplicateResolver(powerOutagePort, sourcePort);
        ...
        lenient().when(sourcePort.findById(sourceA.id())).thenReturn(Optional.of(sourceA));
        lenient().when(sourcePort.findById(sourceB.id())).thenReturn(Optional.of(sourceB));
    }
    ...
    void externalReferenceSameSourceSameRefIsSameIdentity() {
        ParsedOutage po1 = new ParsedOutage(sourceA.id(), start, start.plusSeconds(3600), "r", "ext-123", List.of(new AddressInput("Омская область", null, null, "Омск", null, "ул Ленина", "15")));
    ...
    void externalReferenceDifferentSourceIsDifferentIdentity() {
        ParsedOutage po1 = new ParsedOutage(sourceA.id(), start, start.plusSeconds(3600), "r", "ext-123", List.of(new AddressInput("Омская область", null, null, "Омск", null, "ул Ленина", "15")));
        ParsedOutage po2 = new ParsedOutage(sourceB.id(), start, start.plusSeconds(3600), "r", "ext-123", List.of(new AddressInput("Омская область", null, null, "Омск", null, "ул Ленина", "15")));
        when(powerOutagePort.findBySourceAndExternalReference(sourceA.id(), "ext-123")).thenReturn(Optional.empty());
    ...
    void fallbackSameSourceStartAndAddressesIsSameIdentity() {
        ...
        ParsedOutage po = new ParsedOutage(sourceA.id(), start, start.plusSeconds(3600), "r", null, List.of(new AddressInput("Омская область", null, null, "Омск", null, "ул Ленина", "15")));
    ...
    void fallbackDifferentSourceIsDifferent() {
        ...
        ParsedOutage po1 = new ParsedOutage(sourceA.id(), start, start.plusSeconds(3600), "r", null, List.of(new AddressInput("Омская область", null, null, "Омск", null, "ул Ленина", "15")));
        ParsedOutage po2 = new ParsedOutage(sourceB.id(), start, start.plusSeconds(3600), "r", null, List.of(new AddressInput("Омская область", null, null, "Омск", null, "ул Ленина", "15")));
        resolver.resolve(po1, List.of(address1));
        resolver.resolve(po2, List.of(address1));
        verify(powerOutagePort).findBySourceAndFallbackIdentity(sourceA.id(), start, ids);
    ...
    void fallbackDifferentStartTimeIsDifferent() {
        ...
        ParsedOutage po1 = new ParsedOutage(sourceA.id(), start, start.plusSeconds(3600), "r", null, List.of(new AddressInput("Омская область", null, null, "Омск", null, "ул Ленина", "15")));
        ParsedOutage po2 = new ParsedOutage(sourceA.id(), start2, start2.plusSeconds(3600), "r", null, List.of(new AddressInput("Омская область", null, null, "Омск", null, "ул Ленина", "15")));
        resolver.resolve(po1, List.of(address1));
        resolver.resolve(po2, List.of(address1));
        verify(powerOutagePort).findBySourceAndFallbackIdentity(sourceA.id(), start, ids);
    ...
    void fallbackDifferentAddressesIsDifferent() {
        ...
        when(powerOutagePort.tryCreateWithFallback(any())).thenReturn(new PowerOutagePort.CreateResult(true, existingOutage(sourceA, null))).thenReturn(new PowerOutagePort.CreateResult(true, existingOutage(sourceA, null)));
        ParsedOutage po1 = new ParsedOutage(sourceA.id(), start, start.plusSeconds(3600), "r", null, List.of(new AddressInput("Омская область", null, null, "Омск", null, "ул Ленина", "15")));
        ParsedOutage po2 = new ParsedOutage(sourceA.id(), start, start.plusSeconds(3600), "r", null, List.of(new AddressInput("Омская область", null, null, "Омск", null, "ул Ленина", "15")));
        resolver.resolve(po1, List.of(address1));
    ...
    void endTimeDoesNotChangeIdentity() {
        ...
        ParsedOutage po1 = new ParsedOutage(sourceA.id(), start, end1, "r", null, List.of(new AddressInput("Омская область", null, null, "Омск", null, "ул Ленина", "15")));
        ParsedOutage po2 = new ParsedOutage(sourceA.id(), start, end2, "r", null, List.of(new AddressInput("Омская область", null, null, "Омск", null, "ул Ленина", "15")));
    ...
    void reasonDoesNotChangeIdentity() {
        ...
        when(powerOutagePort.findBySourceAndFallbackIdentity(sourceA.id(), start, ids)).thenReturn(Optional.of(existing));
        when(powerOutagePort.save(any())).thenAnswer(inv -> inv.getArgument(0));
        ParsedOutage poSame = new ParsedOutage(sourceA.id(), start, start.plusSeconds(3600), "old", null, List.of(new AddressInput("Омская область", null, null, "Омск", null, "ул Ленина", "15")));
        ParsedOutage poDiff = new ParsedOutage(sourceA.id(), start, start.plusSeconds(3600), "new", null, List.of(new AddressInput("Омская область", null, null, "Омск", null, "ул Ленина", "15")));
    ...
    void resolutionCreateWhenAbsent() {
        ...
        ParsedOutage po = new ParsedOutage(sourceA.id(), start, start.plusSeconds(3600), "r", "ext-123", List.of(new AddressInput("Омская область", null, null, "Омск", null, "ул Ленина", "15")));
    ...
    void resolutionUpdateWhenChanged() {
        ...
        when(powerOutagePort.save(any())).thenAnswer(inv -> inv.getArgument(0));
        ParsedOutage po = new ParsedOutage(sourceA.id(), start, start.plusSeconds(7200), "new", "ext-123", List.of(new AddressInput("Омская область", null, null, "Омск", null, "ул Ленина", "15")));
        var result = resolver.resolve(po, List.of(address1));
