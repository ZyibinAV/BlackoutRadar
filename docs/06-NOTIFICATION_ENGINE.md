...
- определение включённых каналов доставки;
- выбор Delivery Adapter через Channel Registry;
- передачу готового сообщения Delivery Adapter;
...
1. определяет включённые каналы;
2. для каждого канала определяет Delivery Adapter через Channel Registry;
3. передаёт готовое сообщение выбранному Delivery Adapter;
...
- выполняет техническую операцию доставки;
- возвращает результат
  Notification Engine.

Adapter не изменяет
бизнес-содержание Notification.
...
Готовое сообщение передаётся
соответствующему Delivery Adapter.
